...
const PrivacyPolicy = () => {
    ...
    const fetchPrivacyPolicy = async () => {
        ...
        try {
            ...
            const errorMessage = err.response?.data?.message || err.message || 'Failed to load privacy policy';
    ...
    if (error) {
        return (
            <div className="alert alert-danger m-4" role="alert">
                ...
                <p>{error}</p>
                <button className="btn btn-primary" onClick={fetchPrivacyPolicy}>
                    Try Again
                </button>
                <ToastContainer
                    position="top-right"
                        ...
                        <div className="row">
                            <div className="col-12">
                                <div className="pagebanner-text">
                                    <h1>{privacyContent?.heading || 'Privacy Policy'}</h1>
                                    {privacyContent?.sub_heading && <p>{privacyContent.sub_heading}</p>}
                                </div>
                            </div>
                        </div>
                    </div>
