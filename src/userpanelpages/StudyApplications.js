...
const StudyApplications = () => {
  ...
  const columns = [
    ...
    { id: "total_budget", label: "Budget", render: (app) => (app.total_budget ? `$${app.total_budget}` : "N/A") },
  ...
  const actions = (app) => (
    <>
      <Button
        ...
        to={`/study-application-details/${app.id}`}
        size="small"
        variant="contained"
        color="primary"
      ...
      {app.status === "Pending" ? (
        <Button
          component={RouterLink}
          to={`/edit-study-application/${app.id}`}
          size="small"
          variant="outlined"
          color="secondary"
          sx={{ textTransform: "none", borderRadius: "50px", fontWeight: 600 }}
  ...
  if (loading) {
    return (
      <ProtectedPageLayout fullName={fullName} userId={userId} title="Study Applications">
    ...
    <ProtectedPageLayout fullName={fullName} userId={userId} title="Study Applications">
      <Stack spacing={2.5}>
        ...
        <ApplicationsDataList
          columns={columns}
          rows={applications}
          emptyTitle="No study applications found"
