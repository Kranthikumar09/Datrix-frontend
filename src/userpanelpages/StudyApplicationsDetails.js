    ...
    return (
      <ProtectedPageLayout fullName={fullName} userId={userId}>
        <div className="alert alert-danger" role="alert">
          ...
          <p>{error}</p>
          <Link to="/study-applications" className="btn btn-primary">Back to Applications</Link>
        </div>
        <ToastContainer theme="colored" position="top-right" autoClose={3000} />
      </ProtectedPageLayout>
    );
  ...
  return (
    <ProtectedPageLayout fullName={fullName} userId={userId}>
                      <div className="single-area">
                        <h5 className="mb-0">Study Application Details</h5>
                      </div>
                      <div className="setting-personal-details">
                        ...
                        <div className="details-table">
                          <table className="table table-bordered table-sm">
                            <tbody>
                              ...
                              <tr>
                                <th>Study Level</th>
                                <td>{application.intrested_in_study_level || 'N/A'}</td>
                              </tr>
                              <tr>
                                <th>Stream</th>
                                <td>{application.stream || 'N/A'}</td>
                              </tr>
                              <tr>
                                <th>Percentage</th>
                                <td>{application.percentage ? `${application.percentage}%` : 'N/A'}</td>
                              </tr>
                              <tr>
                                <th>Total Budget</th>
                                <td>{application.total_budget ? `$${application.total_budget}` : 'N/A'}</td>
                              </tr>
                              <tr>
                        ...
                        <div className="single-input btn-gp">
                          ...
                          {application.status === 'Pending' && (
                            <Link to={`/edit-study-application/${application.id}`} className="border-btn btn">
                              Edit Application
                            </Link>
                          )}
                        </div>
